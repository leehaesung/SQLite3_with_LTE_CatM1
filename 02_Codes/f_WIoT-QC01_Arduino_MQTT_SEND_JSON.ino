/**************************************************************************
 ...
 - Added some codes for JSON format:
      ...
      L72: char jsondata[100];
      L198 - L209: Json formatter 
 - Devices: Cat.M1 module(Quectel:WIoT-QC01), Arduino Mega 2560   
 - Reference: https://github.com/Wiznet/wiznet-iot-shield-arduino-kr
***************************************************************************/
...
#define MQTT_CLIENTID                     "clientExample33"
#define MQTT_USERID                       ""
...
#define MQTT_SAMPLE_TOPIC_A               "catm1topic"
...
char mqtt_broker_url[] = "broker.hivemq.com"; //broker.hivemq.com
...
void setup() {
  ...
  if (mqtt_state == MQTT_STATE_CONNECTED) {
    MYPRINTF("[MQTT] Connected. Start to send a message. \r\n");
  }
}
...
void loop() {
  ...
  dtostrf(celsius, 4, 3, mqtt_message);     // Float --> Cstring(Char Array)
  ...
  root["device"] = "catm1";
  root["temperature"] = mqtt_message;
  root.printTo(jsondata); // jsondata: {"device":"catm1","temperature":"26.633"}

  // put your main code here, to run repeatedly:
  // MQTT Publish test

if (sendMqttPublishMessage_BG96(MQTT_SAMPLE_TOPIC_A, MQTT_QOS1, MQTT_RETAIN, jsondata, strlen(jsondata)) == RET_OK) {
    dumpMqttPublishMessage_BG96(MQTT_SAMPLE_TOPIC_A,  mqtt_message);
