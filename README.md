...
SKT텔레콤 LTE Cat.M1은 BG96 IoT통신모듈이 내장된 WIZnet 개발보드와 디지털 온도센서로 사무실 온도를 계측하고 LTE 기지국, 교환기, 응용서버까지 전송한다. MQTT프로토콜로 온도 데이터 수집 및 전송, SQL에 저장, CVS파일 추출, 그리고 데이타분석(시계열분석, RNN, LSTM)을 주피터 노트북에서 딥러닝을 구현함
